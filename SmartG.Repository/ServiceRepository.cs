...
namespace SmartG.Repository
{
    public class ServiceRepository : GenericRepositoryBase<OfferedService>, IServiceRepository
    {
        ...
        {
            return await FindByCondition(s => s.OfferedServiceId.Equals(serviceId), trackChanges).Include(b=>b.ContentBlocks)
                .SingleOrDefaultAsync();
        }

        public async Task<OfferedService> GetServiceBySlugAsync(string slug, bool trackChanges)
        {
            return await FindByCondition(s => s.Slug.Equals(slug), trackChanges).Include(b => b.ContentBlocks)
                .SingleOrDefaultAsync();
        }

        public async Task<PagedList<OfferedService>> GetServicesAsync(RequestParameters requestParameters, bool trackChanges)
        {
            var services = await FindAll(trackChanges).Search(requestParameters.SearchTerm).Include(b => b.ContentBlocks).OrderByDescending(p => p.DateCreated).ToListAsync();
